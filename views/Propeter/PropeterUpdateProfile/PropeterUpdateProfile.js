    ...
    getDocs(collection(db, "Propeter", user.uid, "Private_Data")).
      ...
      then((querySnapshot)=>{
        querySnapshot.forEach((doc)=>{
          ...
          if(doc.data().Foto == ''){
            document.getElementById("Pfp2").src="https://firebasestorage.googleapis.com/v0/b/parkareas-ebf15.appspot.com/o/ImagenesPredeterminadas%2FImagenesPerfil%2Fpic-1.jpg?alt=media&token=49bc1381-0e04-4aba-8351-78090b3a72a1"
          }
          else{
            document.getElementById("Pfp2").src=doc.data().Foto
          }
        })
      })

    updateProfile2.addEventListener("click", (e) => {
      e.preventDefault();
      ...
      if(image2.length!=0){
          ...
          const storageRef = ref(storage, "profileFotos/"+user.uid+'/'+image.files[0].name);
              ...
              getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
                ...
                .then((querySnapshot)=>{
                  querySnapshot.forEach((docC) => {
                    if(docC.data().Id === user.uid){
                        updateDoc(doc(db, "Propeter", user.uid, "Private_Data", docC.id), {
                          Foto: downloadURL,
                        });
                    }
                  })
