        ...
        getDocs(collection(db, "Agendas", "Id_cita", "data_cita")).
            then((querySnapshot) => {
                querySnapshot.forEach((doc4) => {
                    if(doc4.data().Id_parking==localStorage.getItem("id_parking")){
                        ...
                        citasObj.id = doc4.data().Cedula
                        citasObj.start = doc4.data().Fecha
                        citasObj.name = doc4.data().Nombre
                        citasObj.color = doc4.data().Color
                        citas.push(citasObj)
                    }
                })
            }).then(() => {
                ...
                frm.addEventListener("submit", function (e) {
                    ...
                    if (fecha.length != 0 && nombre.length != 0 && telefono.length != 0 && cedula.length != 0 && color.length != 0) {
                        ...
                        if (document.getElementById('titulo').textContent == 'Registrar Cita') {
                            addDoc(collection(db, "Agendas", "Id_cita", "data_cita"), {
                                Cedula: cedula,
                                Fecha: fecha,
                                Nombre: nombre,
                                Telefono: telefono,
                                ...
                                then((querySnapshot)=>{
                                    querySnapshot.forEach((doc2)=>{
                                        if(doc2.data().Id_Parking==localStorage.getItem("id_parking")){
                                            nombreParqueadero = doc2.data().NombreParqueadero
                                    ...
                                    then((querySnapshot)=>{
                                        querySnapshot.forEach((doc3)=>{
                                            updateDoc(doc(db, "Users", user.uid, "Private_Data",doc3.id),{
                                                Afiliacion: nombreParqueadero   
                                            })
                                        })
                                    })
                                ...
                                then((querySnapshot) => {
                                    querySnapshot.forEach((doc2) => {
                                        if (doc2.data().Cedula == cedula) {
                                            updateDoc(doc(db, "Agendas", "Id_cita", "data_cita", doc2.id), {
                                                Cedula: cedula,
                                                Fecha: fecha,
                                                Nombre: nombre,
                                                Telefono: telefono,
                        ...
                        if (result.isConfirmed) {
                            getDocs(collection(db, "Agendas", "Id_cita", "data_cita")).
                                then((querySnapshot) => {
                                    querySnapshot.forEach((doc2) => {
                                        if (doc2.data().Cedula == cedula) {
                                            deleteDoc(doc(db, "Agendas", "Id_cita", "data_cita", doc2.id)).
                                                then(() => {
                                                    myModal.hide();
                                                    calendar.refetchEvents();
                                                }).then(() => {
